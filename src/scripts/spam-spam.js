...
// to target the data for each customer in customers[]: customer.contacts.email[]

// First, map an array of arrays
// This returns an array of the three email address for each customer (10 customers, 3 names each).  It is an array of arrays
const spamBot = customers.map(customer => customer.contacts.email)
console.log("spamBot", spamBot)

// Next, flatten the array by using the 'reduce' method and concatenating all the smaller arrays inside the big array
const reduced = spamBot.reduce((concatArray, currentArray)=>{
return concatArray.concat(currentArray)
})
